...
class _AddPropertyDetailsState extends State<AddPropertyDetails> {
  ...
  void _onTapChooseLocation(FormFieldState state) async {
    ...
    print("object ${widget.propertyDetails?['latitude']}");
    // return;
  ...
  void _onTapContinue() async {
    ...
    var kb = formatFileSize(metaTitle?.lengthSync() ?? 0).kb;
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (check == false) {
        ...
        return;
      } else if (metaTitle == null && metaImageUrl == "") {
        Future.delayed(Duration.zero, () {
          UiUtils.showBlurredDialoge(context,
              sigmaX: 5,
              sigmaY: 5,
      ...
      if (metaImageUrl == "" && kb > 300) {
        HelperUtils.showSnackBarMessage(
            context, "Meta image must be lower than 300KB in size");
